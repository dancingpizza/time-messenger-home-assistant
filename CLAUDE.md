...
- **`docs/index.html` синхронизирован с README.** Это отдельный статический
  ...
  в том же коммите.
- **Нетривиальные изменения документируются вместе с кодом.** В dated spec
  фиксируй не только итог, но и причину решения, рассмотренные альтернативы,
  lifecycle, классификацию ошибок, важные edge cases и способ проверки.
  Неочевидную локальную механику поясняй рядом с кодом; пользовательское
  поведение синхронизируй в README и `docs/index.html`. История чата не
  считается документацией проекта.
